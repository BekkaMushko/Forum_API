...
module.exports = {
  ...
  isLikeCreated: async (req, res) => {
    ...
    try {
      ...
      const result = await new Like().find(post.id, 'post', req.user.id);
      if (result === null) {
        return res.status(500).json({
          status: false,
          error: 'Internal server error'
  ...
  isFavorite: async (req, res) => {
    ...
    try {
      ...
      const result = await new User({ id: req.user.id }).is_post_favorite(post.id);
      if (result === null) {
        return res.status(500).json({
          status: false,
          error: 'Internal server error'
  ...
  isFollowed: async (req, res) => {
    ...
    try {
      ...
      const result = await new User({ id: req.user.id }).is_post_followed(post.id);
      if (result === null) {
        return res.status(500).json({
          status: false,
          error: 'Internal server error'
